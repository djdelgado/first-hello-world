...
_.first = function(arr, num){
    if(Array.isArray(arr) !== true || num < 0){
        return [];
    }else if(Number.isNaN(num) || num === undefined){
        return arr[0];
...
_.filter = function(arr, action){
   var newArr = [];
  _.each(arr, function(item, i, array) {
    if (action(item, i, array)) {
        newArr.push(item);
    }
  });
  return newArr;
...
_.reject = function(arr, action){
     return _.filter(arr, function(item, i, array){
        return !action(item, i, array);
    }); 
};    
...
_.partition = function(arr, action){
    return [_.filter(arr, action), _.reject(arr, action)];
};
//     var dubArr = new Array(2);
//     dubArr[0] = [];
//     dubArr[1] = [];
//     _.each(arr, function(element, key, arr){
//         if(action(element, key, arr)){
//             dubArr[0].push(element);
//         } else {
//             dubArr[1].push(element);
//         }
//     });
//      return dubArr;
// };
...
_.map = function(collection, action){
    var newArr = [];
    _.each(collection, function(item, i, array) {
          newArr.push(action(item, i, array));
    }); return newArr;
...
_.pluck = function(arrObj, key){
    return _.map(arrObj, function(obj, i, array){
       return obj[key]; 
    });
};
...
_.reduce = function(array, action, seed){
    ...
    for (;i < array.length; i++){
        combined = action(combined, array[i], i, array);
    } return combined;
};
...
*/
_.extend = function(myObj){
    var arr = arguments;
    var obj = myObj;
    // if(arguments.length < 2){
    //     obj = Object.assign(arguments[0], arguments[1]);
    // } else {
        for (var i = 0; i <= arr.length; i++){
   ...
   // }
    ...
    // var obj = Object.assign(arguments[0], arguments[1]);
    // if(arguments.length > 2){
